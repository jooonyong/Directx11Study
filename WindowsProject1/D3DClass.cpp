...
bool D3DClass::Initialize(int screenWidth, int screenHeight, bool vsync, HWND hwnd, bool fullscreen, float screenDepth, float screenNear)
{
	...
	//팩토리 객체를 사용하여 첫번쨰 그래픽 카드 인터페이스 어댑터 생성
	...
	//적합한 것을 찾으면 모니터의 새로고침 비율의 분모와 분자 값 저장
	...
	//worldmatrix를 단위행렬로 초기화
	m_worldMatrix = XMMatrixIdentity();
	//2d 렌더링을 위한 직교 투영행렬 생성
	m_orthoMatrix = XMMatrixOrthographicLH((float)screenWidth, (float)screenHeight, screenNear, screenDepth);
	
	//2d 렌더링을 위한 z버퍼를 끄는 두번째 깊이 스텐실 상태 생성.
	//유일한 차이점은 DepthEnable을 false로 설정하면 다른 모든 매개변수는 다른 깊이 스텐실 상태와 동일
...
{
	//종료 전 윈도우 모드로 설정하지 않으면 스왑 체인을 해제할때 예외가 발생함
	if (m_swapChain)
	{
