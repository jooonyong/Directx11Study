...
{
	return InitializeShader(device, hwnd, L"./MultiTextureVS.hlsl", L"./MultiTexturePS.hlsl");
}
...
bool MultiTextureShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	HRESULT result = D3DCompileFromFile(vsFilename, NULL, NULL, "main", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0,
...
void MultiTextureShaderClass::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, WCHAR* shaderFilename)
{
	...
	MessageBox(hwnd, L"Error comiling shader", shaderFilename, MB_OK);
}
